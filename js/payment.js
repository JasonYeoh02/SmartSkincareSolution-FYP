        ...
        if (user) {
            ...
            try {
                ...
                if (billingSnapshot.exists()) {
                    ...
                    if (!billingData || !billingData.cardHolderName || !billingData.cardNumber || !billingData.expiry || !billingData.cvv) {
                        alert('Billing details are incomplete. Please update your profile before proceeding.');
                        window.location.href = '/html/profile.html'; // Redirect to profile page
                        return; // Stop further execution
                    }

                    // Populate billing details if they exist
                    ...
                    alert('Billing details not found. Please update your profile before proceeding.');
                    window.location.href = '/html/profile.html'; // Redirect to profile page
                }
            } catch (error) {
                console.error('Error fetching billing details:', error);
                alert('An error occurred while fetching billing details. Please try again later.');
            }
        } else {
...
confirmPaymentButton.addEventListener('click', async () => {
    try {
        ...
        if (!cardHolderName || !cardNumber || !expiryDate || !cvv) {
            alert('Payment details are incomplete. Please update your billing information.');
            modal.style.display = 'none'; // Hide modal
            return;
        }
